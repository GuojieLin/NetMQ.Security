        ...
        public void Do2()
        {
            ...
            using (var socket = new StreamSocket())
            {
                ...
                using (SecureChannel secureChannel = SecureChannel.CreateClientSecureChannel(null, m_configuration))
                {
                    ...
                    List<RecordLayer> outRecordLayers = new List<RecordLayer>();

                    //客户端发送client hello
                    bool clientComplete = secureChannel.ProcessMessage(null, outRecordLayers);

                    SendMessages(socket, outRecordLayers);
                    bool done = false;
                    ...
                    do
                    {
                        ...
                        //SplitInMessage
                        done = secureChannel.ResolveRecordLayer(buffer, null, outRecordLayers);
                        if (outRecordLayers != null)
                        {
                            SendMessages(socket, outRecordLayers);
                        }
                        if (buffer.Length == 0)
                    ...
                    //握手完成
                    SendMessages(socket, outRecordLayers);
                    List<RecordLayer> resolveRecordLayers = new List<RecordLayer>();
                    for (int i = 0; i < 10; i++)
                    {
                        outRecordLayers.Clear();
                        ...
                        var recordLayer = secureChannel.EncryptApplicationData(new ReadonlyBuffer<byte>(Encoding.GetEncoding("GBK").GetBytes(length + str)));
                        ...
                        {
                            foreach (var recordProtocol in message.RecordProtocols)
                            {
                                // decrypting the message
                                byte[] plainMessage = secureChannel.DecryptApplicationData(recordProtocol.HandShakeData);
